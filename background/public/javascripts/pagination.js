...
!function(){
	...
	Page.prototype={
		...
		init:function(config){
			...
			this.type = config.type || 0;//当前页面pageNum名称
			this.create();
		},
		create:function(){
				...
				if(aNum>0){
					...
					if(n>=0){
						for(var i=0;i<this.num;i++){
							...
							a+= "<a href='"+this.href+pagenumber+"' onclick=setCurpage("+pagenumber+","+this.type+") class="+(i==0?'active':'')+">"+pagenumber+"</a>";
						}	
					}else{
						for(var i=n;i<this.count-this.page+1;i++){
							a+= "<a href='"+this.href+(this.page+i)+"' onclick=setCurpage("+(this.page+i)+","+this.type+") class="+(this.page+i==this.page?'active':'')+">"+(this.page+i)+"</a>";
						}	
					}	
				}else{
					for(var i=0;i<this.count;i++){
						a+= "<a href='"+this.href+(1+i)+"' onclick=setCurpage("+(1+i)+","+this.type+") class="+(this.page==(1+i)?'active':'')+">"+(1+i)+"</a>";
					}	
				}
				var preurl='',nexturl='';
				// preurl=this.page==1?this.href+this.page:this.href+(--this.page);
				if(this.page==1){
					preurl=this.href+this.page
				}else if(1 < this.page-1-this.step){
					preurl=this.href+(this.page-this.step)
				}else{
					preurl=this.href+1
				}
				// nexturl=this.page==this.count?this.href+this.page:this.href+(++this.page+this.step);
				if(this.page==this.count){
					nexturl=this.href+this.page
				}else if(this.count > 1+this.page+this.step){
					nexturl=this.href+(this.page+this.step)
				}else{
					nexturl=this.href+this.count
				}
				dom = '<div class="page-contain"><a href="'+preurl+'" onclick=setCurpage('+preurl+','+this.type+')><</a><div class="page-box">'+a+'</div><a href="'+nexturl+'" onclick=setCurpage('+nexturl+','+this.type+')>></a></div>';	
			}else{
				if(aNum<0){
					for(var i=0;i<this.count;i++){
